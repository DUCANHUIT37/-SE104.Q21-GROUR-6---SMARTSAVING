...
CREATE TABLE LoaiTietKiem (
    ...
    ky_han_thang              INT             NOT NULL DEFAULT 0
                                COMMENT '0 = không kỳ hạn',
    lai_suat_nam              DECIMAL(6,4)    NOT NULL
                                COMMENT 'Lãi suất %/năm, vd: 0.0050 = 0.5%',
    so_tien_gui_toi_thieu     DECIMAL(18,0)   NOT NULL DEFAULT 1000000
                                COMMENT 'Tiền gửi ban đầu tối thiểu (QĐ1)',
    dang_ap_dung              TINYINT(1)      NOT NULL DEFAULT 1
                                COMMENT '0 = vô hiệu, không cho mở sổ mới',
    tao_boi                   INT             NULL,
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_loaitk_taoboi
        FOREIGN KEY (tao_boi) REFERENCES NguoiDung(id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_loaitk_capnhatboi
...
CREATE TABLE SoTietKiem (
    ...
    lai_suat_mo_so        DECIMAL(6,4)    NOT NULL
                            COMMENT 'Snapshot lãi suất lúc mở sổ - không đổi dù admin chỉnh QĐ sau',
    ngay_mo               DATE            NOT NULL,
    ngay_dao_han          DATE            NULL
                            COMMENT 'NULL nếu không kỳ hạn',
    trang_thai            ENUM(
                            'dang_hoat_dong',
                            'da_tat_toan'
                          )               NOT NULL DEFAULT 'dang_hoat_dong',
    tao_luc               DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (id),
    UNIQUE KEY uq_sotk_maso (ma_so),
    CONSTRAINT fk_sotk_khachhang
        FOREIGN KEY (khach_hang_id) REFERENCES NguoiDung(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_sotk_loaitk
        FOREIGN KEY (loai_tiet_kiem_id) REFERENCES LoaiTietKiem(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_sotk_nhanvientao
        FOREIGN KEY (nhan_vien_tao_id) REFERENCES NguoiDung(id)
        ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE PhieuGoi (
    ...
    so_tien_goi       DECIMAL(18,0)   NOT NULL
                        COMMENT 'Tối thiểu 100.000đ theo QĐ2',
    ...
    CONSTRAINT fk_phieugoi_sotk
        FOREIGN KEY (so_tiet_kiem_id) REFERENCES SoTietKiem(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_phieugoi_nhanvien
        FOREIGN KEY (nhan_vien_id) REFERENCES NguoiDung(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT chk_phieugoi_sotien
        CHECK (so_tien_goi >= 100000)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE PhieuRut (
    ...
    tien_lai_tinh         DECIMAL(18,0)   NOT NULL DEFAULT 0
                            COMMENT 'Tiền lãi = số dư × lãi suất áp dụng (QĐ3)',
    lai_suat_ap_dung      DECIMAL(6,4)    NOT NULL
                            COMMENT 'Nếu rút trước hạn → dùng lãi suất không kỳ hạn',
    ngay_rut              DATE            NOT NULL,
    tat_toan              TINYINT(1)      NOT NULL DEFAULT 0
                            COMMENT '1 = rút hết, sổ tự đóng (QĐ3)',
    ghi_chu               VARCHAR(500)    NULL,
    tao_luc               DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (id),
    UNIQUE KEY uq_phieurut_maphieu (ma_phieu),
    CONSTRAINT fk_phieulrut_sotk
        FOREIGN KEY (so_tiet_kiem_id) REFERENCES SoTietKiem(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_phieulrut_nhanvien
        FOREIGN KEY (nhan_vien_id) REFERENCES NguoiDung(id)
        ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE LichSuGiaoDich (
    ...
    so_tien           DECIMAL(18,0)   NOT NULL
                        COMMENT 'Dương = tiền vào, âm = tiền ra',
...
('admin@smartsavings.com', '$2a$10$5ShPaYZQ0bbqGk.zNuZ/kuY2TAcpQ2xKcQuEXyf1VeJIiDf6SFcra', 'quan_tri_vien', 1);