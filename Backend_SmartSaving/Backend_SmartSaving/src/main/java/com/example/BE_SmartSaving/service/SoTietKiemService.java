...
public class SoTietKiemService {
    ...
    public SoTietKiem moSoTietKiem(SoTietKiem soTietKiem) {
        ...
        LoaiTietKiem loai = loaiTietKiemService.layTheoId(
    ...
    public List<SoTietKiem> timKiem(String tuKhoa) {
        return soTietKiemRepository.timKiem(tuKhoa);
    }

    public List<SoTietKiem> laySoTheoKhachHang(Integer khachHangId) {
        return soTietKiemRepository.findByKhachHangId(khachHangId);
    }

    /**
     * Chỉ cho phép xoá sổ đã tất toán.
     * Sổ đang hoạt động KHÔNG được xoá (yêu cầu an toàn dữ liệu).
     */
    public void xoaSo(Integer id) {
        SoTietKiem stk = laySoTheoId(id);
