...
class BaoCaoServiceTest {
    ...
    void baoCaoThang_loaiIdCuThe_chiLayMotLoai() {
        ...
        when(soTietKiemRepository.demSoMoTheoLoaiVaNgay(eq(1), any())).thenReturn(0L);
        when(phieuRutRepository.demSoDongTheoLoaiVaNgay(eq(1), any())).thenReturn(0L);
    ...
    void baoCaoThang_loaiIdNull_layTatCaLoai() {
        YearMonth thang = YearMonth.of(2026, 5);
        when(loaiTietKiemRepository.findAll()).thenReturn(List.of(loai1, loai2));
        when(soTietKiemRepository.demSoMoTheoLoaiVaNgay(anyInt(), any())).thenReturn(0L);
        when(phieuRutRepository.demSoDongTheoLoaiVaNgay(anyInt(), any())).thenReturn(0L);
    ...
    void baoCaoThang_ngayCoPhaSinh_themVaoDanhSach() {
        ...
        when(soTietKiemRepository.demSoMoTheoLoaiVaNgay(eq(1), any())).thenReturn(0L);
        when(phieuRutRepository.demSoDongTheoLoaiVaNgay(eq(1), any())).thenReturn(0L);
        when(soTietKiemRepository.demSoMoTheoLoaiVaNgay(1, ngayCoMo)).thenReturn(3L);
        when(phieuRutRepository.demSoDongTheoLoaiVaNgay(1, ngayCoMo)).thenReturn(1L);

        List<BaoCaoThangDTO> result = baoCaoService.lapBaoCaoMoDongSoThang(thang, null);

        assertThat(result).hasSize(1);
        BaoCaoThangDTO dto = result.get(0);
        assertThat(dto.getNgay()).isEqualTo(ngayCoMo);
